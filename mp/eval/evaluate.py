...
def dl_losses(dl, agent, loss_f):
    ...
    for data in dl:
        ...
        #torchio.Image(tensor=inputs.cpu().detach().numpy()).save("M:/test_img_" + "AAA" + ".nii.gz")
        #torchio.Image(tensor=targets.cpu().detach().numpy()).save("M:/test_mask_" + "AAA" + ".nii.gz")
        #torchio.Image(tensor=outputs.cpu().detach().numpy()).save("M:/test_pred_" + "AAA" + ".nii.gz")
        # Calculate losses
