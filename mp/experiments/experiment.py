...
class Experiment:
    ...
    def set_data_splits(self, data):
        try:
            ...
            self.new_data_splits(data)

    def new_data_splits(self, data):
        print('Dividing dataset')
    ...
    # TODO REPLACE
    def set_data_splits_second(self, data):
        try:
            self.splits_second = lr.load_json(path=self.path, name='splits_second')
            print('Loading splits')
            print('Dividing dataset')
        except FileNotFoundError:
            self.new_data_splits_second(data)

    def new_data_splits_second(self, data):
        print('Dividing dataset')
        # If the data consists of several datasets, then the splits are a
        # dictionary with one more label, that of the dataset name.
        if isinstance(data, Data):
            self.splits_second = dict()
            for ds_name, ds in data.datasets.items():
                self.splits_second[ds_name] = split_dataset(ds, test_ratio=self.config.get('test_ratio', 0.0), 
                val_ratio=self.config['val_ratio'], nr_repetitions=self.config['nr_runs'], 
                cross_validation=self.config['cross_validation'])
        else:
            self.splits_second = split_dataset(data, test_ratio=self.config.get('test_ratio', 0.0), 
                val_ratio=self.config['val_ratio'], nr_repetitions=self.config['nr_runs'], 
                cross_validation=self.config['cross_validation'])
        lr.save_json(self.splits_second, path=self.path, name='splits_second')
        print('\n')
...
class ExperimentRun:
    ...
    def load_results(self):
        return lr.pkl_load(path=self.paths['results'], name='results')
